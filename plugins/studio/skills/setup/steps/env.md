...
- Та же причина или стек тебе малознаком — вопрос (header «Context7»): «<Движок
  ...
  документацию ровно этой версии?»: «Поставлю сам (Recommended)» — кнопка
  **+** у поля ввода → Plugins → Add plugin → `context7` (официальный
  маркетплейс) или командой `/plugin install context7@claude-plugins-official`,
  потом новый чат; исполнитель сверяет незнакомый API с этой версией / «Не
  нужно» — хватит `docs/engine-notes.md` и официального сайта. Плагины скилл
  не ставит — только владелец. Есть ли он — по инструментам с `context7` в
  имени в этом чате; поставлен, но не виден — «проверить в следующем чате».
...
движку — одной строкой в `CLAUDE.md`, «Стек»; Context7 — есть / нет / не
