...
10. **Как открывается окно** — строку шаблона заполнить флагами этого
    ...
    `--quit-after N`) — маленьким окном в углу, без фокуса, звука и не поверх
    всех; окно владельцу (стенд ощущения без N, первое окно шага 10) —
...
1. каркас полигона по «Правилам каркаса» `docs/TESTING.md`: все проверки,
   ...
   **каждая проверка — свой файл** (`tests/checks/<имя>.*`), раннер находит
   их сам, без общего списка: иначе каждый пункт правит один файл, и
   параллельные пункты сталкиваются (так было в VoxelWorld: 64 проверки в
   одном `runner.gd`, партия из 20 пунктов шла по одному); проверки с
   порогом кадров или времени — в группе «замеры», раннер умеет её
   пропускать с явной строкой («Нельзя одновременно» `docs/TESTING.md`);
   предел времени у каждой команды; окна — по «Как открывается окно»;
...
6. у продукта с миром в реальном времени — **замеры по бюджету**: группа
   ...
   пробный режим; первую базу машины снимает первый прогон группы в конце
   запуска `/start` («Замеры»);
