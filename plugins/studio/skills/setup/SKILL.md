...
3. **Проверка компьютера** — `steps/env.md`: `env_check.ps1`; недостающее для
   ...
   лежит / Пропустить»; итог — `## Окружение` в `docs/TESTING.md`, путь к
   движку — строкой в `CLAUDE.md`. Версия движка — из установленного.
...
9. **Очередь — вертикальный срез** — `steps/plan.md`: проба стека (если
   ...
   коробка → первый кадр по образцу `[вид]` → один полный цикл на 3–5 минут.
...
- как работать дальше: **два чата в этой папке** — назвать их «<Проект> ·
  ...
  (`/start`, `/done`, `/add`, `/check`, `/parallel`, `/retro`); `/board` — в
  любом. Первый шаг: в чате разработки `/start` — он возьмёт первый пункт
  очереди.
