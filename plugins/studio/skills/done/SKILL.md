...
- **паспорта `docs/refs/<тема>.md`** — принятый `[вид]`: `Состояние: принят кадр
  ...
  <составляющие>: «слова владельца»`; ответы выбора, которые `/start` оставил в
  `docs/BATCH.md`, — туда же. `[вид]` `не выполнен: на пересмотр образца` —
  паспорт в `черновик` (и в «Темах»; прежнее состояние — строкой в «Журнал»),
  пункт в очереди — `[ждёт образца]`, в «Решения» `BLOCKED.md` — «Образец
  <тема>: пересмотреть — `/idea образец <тема>`»;
