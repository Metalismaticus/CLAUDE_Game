...
вас», «В документы при `/done`»; строки «Решил сам» раздела «Принято по
...
- «Что из этого вам не нравится? (решено без вас, видно в продукте)» —
  ...
  и видные в продукте строки «Решил сам» из «Принято по умолчанию»
  (оспорена — как «Решено за вас» её пункта): label — номер пункта и что
  видно, словами продукта, без чисел и терминов («П.4: контур темнее
  модели»); description — «сейчас: …; если поменять: …».
  Строка без метки — переписать словами продукта; не сказать, что видит
  владелец, — это `[техника]`. `[техника]` и порядок работы («сначала
  форма», «пункт 3 до выбора пункта 2») окном **не спрашивать**: одной
  ...
  ничего не оспорено; выбран вместе со строками — оспорены строки. Таких
  строк нет — вопроса нет; строка одна — «Пункт N, <что видно>: так
...
назвать зависимость и спросить окном. Строка «Решил сам: пункт M — до выбора
...
оставить: на них ведут строки «Не принят». Отложенное пункта, ушедшего в
...
проверки), файлы поимённо; отправить (`git push`), если в `CLAUDE.md` коммиты
отправляются.
