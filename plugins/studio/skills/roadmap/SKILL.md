...
8. **Записать** после «Принять» (сначала `git status` и `git log` этих
   ...
   «Удалить» — в тот же коммит. Документальный коммит, файлы поимённо;
   отправить, если в `CLAUDE.md` коммиты отправляются; план удалить. Отчёт:
   что увидит игрок в первом этапе, этапы одной строкой, покрытие, что ушло
   в «Потом» и «Не делаем»; дальше — `/start` в чате разработки.
...
3. Куски — в «Подробности», пункты `[этап N]` — в «Очередь» (проба риска —
   ...
   строку «Сейчас: …» поправить.
4. Скрипт, документальный коммит поимённо; отчёт: что увидит игрок, первые
   пункты, дальше — `/start` в чате разработки.
