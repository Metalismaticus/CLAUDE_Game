...
- Каждый раз — `docs/BATCH.md`, разделы «Пришло» и «Найдено по ходу»: их строки
  ...
  ещё нет (сверить по пути файла или сути) — это часть разбора. **Владельца о
  них не спрашивать**: технические находки (проверки, баги, замеры, чистка
  кода, сломанная оснастка) ставит сам чат и называет одной строкой в
  «Принято по умолчанию» («из заметок разработки в очередь: 4 пункта — …»).
  В окно — только находка, которая меняет то, что видит или делает игрок,
  состав этапа, деньги или необратимое. Строка
...
- `multiSelect` «Какие пункты принимаете?» — **только пункты из слов
  ...
  умолчанию», без окна); вариант на пункт, description —
  куда ляжет, последним — `Ничего из этого`; больше трёх пунктов —
  несколькими вопросами по 2–3 пункта («… пункты 1–3?», «… пункты 4–6?»);
  пункт один — «Пункт 1, <суть>: записать так?»;
