...
В сообщении координатора: номера пунктов партии из `docs/BATCH.md` и предел —
сколько пунктов может идти одновременно. Предел из сообщения главнее записанного
в `docs/TESTING.md`.
...
- в `docs/TESTING.md` параллельная работа в состоянии «проверено», а предел
  больше 1;
...
волна, один. Размер волны — не больше предела. В волну из трёх и больше
пунктов правило выполняется **для каждой пары**: пункт с пересечением хотя бы с
одним из них ждёт следующей волны. Больший предел не повод собирать волну
любой ценой — не нашлось независимых, значит их нет. Порядок
