...
3. Для каждого пункта ответить:
   ...
   - **зависимость** — нужен ли ему результат другого пункта партии (`[вид]`
     и `[ощущение]` — от пункта стенда партии, если он есть; двух
     `[ощущение]` одной темы в партии быть не должно — попались: второй
     «зависит от M: ждёт следующей партии»); M ждёт выбора владельца —
     зависит ли от выбора (оси вариантов назовёт координатор) или только от
   ...
   - **особый род** — замер производительности или времени (одновременные
     ...
     работа с данными пользователя, пункт `[вид]` или `[ощущение]` (идёт
     кругами и ждёт выбора владельца между ними; стенд — по одному);
...
любой ценой — не нашлось независимых, значит их нет. Порядок волн — порядок
очереди: параллельность не повод поднимать пункт выше.
...
Узкое место: <один общий файл, из-за которого по одному идут три и больше пунктов, и как его разгрузить> | нет
```
...
похожий. Координатор кладёт это строкой в «Найдено по ходу»: пункт очереди
ставит чат замысла.
