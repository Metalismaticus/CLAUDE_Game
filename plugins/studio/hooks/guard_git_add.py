...
и `git commit -a`. Код 2 блокирует вызов, текст из stderr уходит модели.
Любая собственная ошибка хука — код 0: он не должен мешать работе.
"""
import json
import os
import re
...
ADD_ALL = re.compile(r"\bgit\s+(?:-C\s+\S+\s+)?add\s+(?:\S+\s+)*?(?:-A|--all|\.)(?=\s|$|[;&|])")
COMMIT_ALL = re.compile(r"\bgit\s+(?:-C\s+\S+\s+)?commit\s+(?:\S+\s+)*?(?:-[b-zB-Z]*a[a-zA-Z]*|--all)(?=\s|$|[;&|])")
